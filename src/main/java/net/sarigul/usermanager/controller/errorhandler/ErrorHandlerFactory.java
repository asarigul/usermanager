...
public final class ErrorHandlerFactory {
	...
	public static ErrorHandler<? extends ApplicationException> get(Throwable cause, Controller controller) {
		if(cause instanceof ApplicationException) {
			...
			if(cause instanceof IndexViolationException) {
				return new IndexViolationErrorHandler(controller, (IndexViolationException) cause);
			}
			
			throw new RuntimeException("unhandled subtype: " + cause);
		}
